...
def main(args):
    ...
    for level in levels:
        ...
        log_file.write(f"  {level} Total Average Count: {total_average_count:.2f} Average Total Average Count: {total_average_count_per_video:.2f}\n")
        log_file.write(f"  {level} Total Average Memory Length: {total_average_mem_len:.2f} Average Total Average Memory Length: {total_average_mem_len_per_video:.2f}\n")
        log_file.write(f"  {level} Total Average Percentage: {(total_average_percentage_per_video):.3f}%\n")
        log_file.write(f"  {level} Total Average Memory Percentage: {(total_average_mem_percentage_per_video):.3f}%\n")
        log_file.write(f"  {level} Max Count: {max(info[f'average_count_{level}'] for info in average_video_info.values())} video(s) with max count: {[info['video_name'] for info in average_video_info.values() if info[f'average_count_{level}'] == max(info[f'average_count_{level}'] for info in average_video_info.values())]} \n")
        log_file.write(f" {level} Max Memory Length: {max(info[f'average_mem_len_{level}'] for info in average_video_info.values())} video(s) with max memory length: {[info['video_name'] for info in average_video_info.values() if info[f'average_mem_len_{level}'] == max(info[f'average_mem_len_{level}'] for info in average_video_info.values())]} \n")
        log_file.write(f"  {level} Max Percentage: {max(info[f'average_percentage_{level}'] for info in average_video_info.values()):.2f}% video(s) with max percentage: {[info['video_name'] for info in average_video_info.values() if info[f'average_percentage_{level}'] == max(info[f'average_percentage_{level}'] for info in average_video_info.values())]} \n")
        log_file.write(f"  {level} Max Memory Percentage: {max(info[f'average_mem_percentage_{level}'] for info in average_video_info.values()):.2f}% video(s) with max memory percentage: {[info['video_name'] for info in average_video_info.values() if info[f'average_mem_percentage_{level}'] == max(info[f'average_mem_percentage_{level}'] for info in average_video_info.values())]} \n")
    for _, info in average_video_info.items():
        print(f"Video {info['video_num']}: {info['video_name']}")
        log_file.write(f"Video {info['video_num']}: {info['video_name']}\n")
        for level in levels:
            ...
            log_file.write(f"  Average Count {level}: {info[f'average_count_{level}']:.2f}\n")
            log_file.write(f"  Average Ratio {level}: {info[f'average_ratio_{level}']:.4f} ({info[f'average_percentage_{level}']:.2f}%)\n")
            log_file.write(f"  Average Memory Length: {info[f'average_mem_len_{level}']:.2f}\n")
            log_file.write(f"  Average Memory Ratio: {info[f'average_mem_ratio_{level}']:.4f} ({info[f'average_mem_percentage_{level}']:.2f}%)\n")
    log_file.close()
    ...
    plt.tight_layout()
    plt.savefig(f'Feature_Numbers_Trace_by_Level_Video_{start_time}.png')
    fig, axes = plt.subplots(len(levels), 1, figsize=(10, 10), sharex=True)
    for i, level in enumerate(levels):
        video_nums = [v['video_num'] for v in average_video_info.values()]
    ...
    plt.tight_layout()
    plt.savefig(f'Feature_Percentage_Trace_by_Level_Video_{start_time}.png')
    ...
    plt.tight_layout()
    plt.savefig(f'Memory_Usage_Trace_by_Level_Video_{start_time}.png')
    ...
    plt.tight_layout()
    plt.savefig(f'Memory_Usage_Percentage_Trace_by_Level_Video_{start_time}.png')
