...
class VIDEvaluator:
    ...
    def __init__(
        ...
        # degradation-group evaluation
        blur_map = kwargs.get('blur_map', {})
        self.degradation_map = {}
        for full_key, group in blur_map.items():
            parts = full_key.replace("\\", "/").split("/")
            video_name = parts[-2]
            frame_name = os.path.splitext(parts[-1])[0]
            short_key = f"{video_name}/{frame_name}"
            self.degradation_map[short_key] = group        
...
#        ):
        ...
        if is_main_process() and self.degradation_map:
            degradation_summary = self.evaluate_degradation_subsets(data_list)
    ...
    def calculate_subset_metrics(self, subset_dt, subset_gt, subset_imgs):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json') as tmp_gt:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json') as tmp_dt:
                ...
                if not subset_dt:
                    logger.warning("No detections found for this degradation group.")
                    return 0.0, 0.0
                ...
                if hasattr(cocoEval, 'stats') and len(cocoEval.stats) > 0:
                    ...
                    logger.error("COCOeval.stats is empty after summarize().")
                    return 0.0, 0.0
    ...
    def convert_to_coco_format(self, outputs, info_imgs, labels, paths):
        ...
        for (output, info_img, _label, full_path) in zip(
            ...
            self.vid_to_coco['images'].append({'id': self.id, 'file_name': path_key, 'degradation_group': degradation_group})
